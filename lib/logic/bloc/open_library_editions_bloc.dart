...
class OpenLibraryEditionsBloc
  ...
  final BehaviorSubject<List<OpenLibraryEditionResult>> _editionsListFetcher =
      BehaviorSubject<List<OpenLibraryEditionResult>>();
  Stream<List<OpenLibraryEditionResult>> get editionsList =>
      _editionsListFetcher;
  ...
  OpenLibraryEditionsBloc(
    ...
    on<LoadApiEditionsEvent>((event, emit) async {
      ...
      if (_editionsListFetcher.hasValue) {
        List<OpenLibraryEditionResult> list = _editionsListFetcher.value;
        list.add(result);
        _editionsListFetcher.sink.add(list);
      } else {
        final list = List<OpenLibraryEditionResult>.empty(growable: true);
        list.add(result);
        _editionsListFetcher.sink.add(list);
      }
