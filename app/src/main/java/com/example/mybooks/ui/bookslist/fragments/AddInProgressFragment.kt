...
import kotlinx.android.synthetic.main.fragment_add_in_progress.*

class AddInProgressFragment : Fragment(R.layout.fragment_add_in_progress) {
        ...
        btnAddBook.setOnClickListener { view ->
            ...
            if (bookTitle.isNotEmpty()) {
                if (bookAuthor.isNotEmpty()){
                    val item = Book(bookTitle, bookAuthor, bookRating = 0.0F, bookStatus = "in_progress", bookPriority = "none", bookStartDate = "none", bookFinishDate = "none")
                    viewModel.upsert(item)

                    etAuthor.text.clear()
                    etTitle.text.clear()

                    findNavController().navigate(R.id.action_addInProgressFragment_to_inProgressFragment)
                }
                else {
